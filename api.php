...
try {
    ...
    $check_admin = $pdo->query("SELECT id FROM users WHERE phone='09307446295' LIMIT 1")->fetch();
    if (!$check_admin) {
        $pdo->exec("INSERT INTO `users` (`phone`, `password_hash`, `full_name`, `role`, `is_super_admin`)
        VALUES ('09307446295', '$2y$10\$yFEvqg7.k4lGZp.3mGgW/OQv1bWeB4dF1lX.2wIenjDszk9u6D/K.', 'مدیر کل سامانه', 'admin', 1)");
    }
...
$db['orders'] = array_merge($mappedRepairs, $db['orders']);
    ...
    case (preg_match('/api\/save-database$/', $path) && $method === 'POST'):
        try {
            ...
            $replaceableKeys = ['orders', 'technicians', 'errorCodes', 'spareParts', 'partPurchases', 'userFeedbacks', 'messages', 'userComments', 'products', 'notifications', 'repairRequests'];
